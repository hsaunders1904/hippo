...
Foam::fvMesh read_polymesh(const std::string &case_dir) {
  cArgs args("foamRun");
  args.push_arg("-case");
  args.push_arg(case_dir);

  int argc = args.get_argc();
  auto argv_vec = args.get_argv();
  char **argv = argv_vec.data();
  Foam::argList arg_list(argc, argv, false, false, false);
  Foam::Time run_time(Foam::Time::controlDictName, arg_list);

  Foam::IOobject mesh_header(Foam::fvMesh::defaultRegion, run_time.name(),
                             run_time, Foam::IOobject::MUST_READ);
  return Foam::fvMesh(mesh_header);
}

} // namespace

InputParameters FoamMesh::validParams() {
  auto params = MooseMesh::validParams();
  params.addRequiredParam<std::vector<std::string>>(
      "foam_patch", "Name of foam boundary patches to replicate.");
  params.addRequiredParam<std::string>(
      "case", "The directory containing the OpenFOAM case.");
  return params;
}
...
FoamMesh::FoamMesh(InputParameters const &params)
    : MooseMesh(params),
      ...
      _foam_mesh(read_polymesh(params.get<std::string>("case"))) {
...
std::unique_ptr<MooseMesh> FoamMesh::safeClone() const {
  return std::make_unique<FoamMesh>(*this);
}

std::vector<int> &FoamMesh::getSubdomainList() { return _subdomain_list; }

void FoamMesh::buildMesh() {
  auto comm = _communicator.get();
...
libMesh::Elem *FoamMesh::getElemPtr(int rank_local) const {
