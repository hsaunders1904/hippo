...
bool is_constant_monomial(const MooseVariableFieldBase &var) {
  return var.order() == libMesh::Order::CONSTANT &&
         var.feType().family == FEFamily::MONOMIAL;
}

Real variableValueAtElement(const libMesh::Elem *element,
...
InputParameters FoamProblem::validParams() {
...
FoamProblem::FoamProblem(InputParameters const &params)
    : ExternalProblem(params),
      ...
      _solver(params.get<bool>("solve")
                  ? Foam::solver::New("fluid", _foam_mesh->fv_mesh()).ptr()
                  : nullptr) {
  assert(_foam_mesh);
  ...
  if (t_var_name.empty() && hf_var_name.empty()) {
    mooseWarning("Neither parameters '", PARAM_VAR_T, "' or '", PARAM_VAR_HF,
                 "' are set. No quantities are being transferred to OpenFOAM.");
  } else if (t_var_name == hf_var_name) {
    mooseError("Parameters '", PARAM_VAR_T, "' and '", PARAM_VAR_HF,
               "' cannot refer to the same variable: '", t_var_name, "'.");
  }
  ...
  if (foam_t_var_name.empty() && foam_hf_var_name.empty()) {
    mooseWarning("Neither parameters '", PARAM_VAR_FOAM_T, "', or '",
                 PARAM_VAR_FOAM_HF,
                 "' are set. No quantities are being copied from OpenFOAM.");
  } else if (foam_t_var_name == foam_hf_var_name) {
    mooseError("Parameters '", PARAM_VAR_FOAM_T, "' and '", PARAM_VAR_FOAM_HF,
               "' cannot refer to the same variable: '", foam_t_var_name, "'.");
  }
}

void FoamProblem::externalSolve() {
  if (parameters().get<bool>("solve")) {
    _solver.run();
  }
}

void FoamProblem::syncSolutions(Direction dir) {
  if (!parameters().get<bool>("solve")) {
    return;
  }
  if (dir == ExternalProblem::Direction::FROM_EXTERNAL_APP) {
    auto transfer_wall_temp =
        !parameters().get<std::string>(PARAM_VAR_FOAM_T).empty();
    auto transfer_wall_heat_flux =
    ...
    auto transfer_wall_temp =
        !parameters().get<std::string>(PARAM_VAR_T).empty();
    auto transfer_wall_heat_flux =
...
void FoamProblem::syncFromOpenFoam() {
  constexpr bool transfer_wall_temp =
      (sync_vars == SyncVariables::WallTemperature) ||
      (sync_vars == SyncVariables::Both);
  constexpr bool transfer_wall_heat_flux =
      (sync_vars == SyncVariables::WallHeatFlux) ||
      (sync_vars == SyncVariables::Both);
  ...
  if constexpr (transfer_wall_heat_flux) {
    wall_heat_flux_var =
        getConstantMonomialVariableFromParameters(PARAM_VAR_FOAM_HF);
  }
  ...
  std::exclusive_scan(patch_counts.begin(), patch_counts.end(),
                      patch_counts.begin(), 0);
  ...
  for (auto i = 0U; i < subdomains.size(); ++i) {
    ...
    for (auto elem = patch_counts[i]; elem < patch_counts[i + 1]; ++elem) {
      ...
      if constexpr (transfer_wall_temp) {
        ...
        auto dof_t =
            elem_ptr->dof_number(sys.number(), wall_temp_var->number(), 0);
        sys.solution().set(dof_t, wall_temp[elem]);
      }
      if constexpr (transfer_wall_heat_flux) {
        auto &sys = wall_heat_flux_var->sys();
        auto dof_dt =
            elem_ptr->dof_number(sys.number(), wall_heat_flux_var->number(), 0);
...
template <FoamProblem::SyncVariables sync_vars>
void FoamProblem::syncToOpenFoam() {
  constexpr bool transfer_wall_temp =
      (sync_vars == SyncVariables::WallTemperature) ||
      (sync_vars == SyncVariables::Both);
  constexpr bool transfer_wall_heat_flux =
  ...
  if constexpr (transfer_wall_heat_flux) {
    wall_heat_flux_var =
        getConstantMonomialVariableFromParameters(PARAM_VAR_HF);
  }
  ...
  for (auto i = 0U; i < subdomains.size(); ++i) {
    ...
    for (size_t elem = patch_counts[i]; elem < patch_counts[i + 1]; ++elem) {
      ...
      if constexpr (transfer_wall_heat_flux) {
        moose_hf.emplace_back(
            variableValueAtElement(elem_ptr, wall_heat_flux_var));
      }
    }
...
MooseVariableFieldBase *FoamProblem::getConstantMonomialVariableFromParameters(
  ...
  if (!is_constant_monomial(*var)) {
    mooseError("variable assigned to parameter '", parameter_name,
               "' must have:\n"
               "  family = MONOMIAL\n"
               "  order = CONSTANT\n");
  }
