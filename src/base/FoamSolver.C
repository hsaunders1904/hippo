...
void adjustDeltaT(Foam::Time &runTime, const Foam::solver &solver) {
  ...
  if (runTime.controlDict().lookupOrDefault("adjustTimeStep", false) &&
    ...
    const Foam::scalar deltaT =
        std::min(solver.maxDeltaT(), runTime.functionObjects().maxDeltaT());

    if (deltaT < Foam::rootVGreat) {
      runTime.setDeltaT(
          std::min(Foam::solver::deltaTFactor * runTime.deltaTValue(), deltaT));
...
std::size_t FoamSolver::appendPatchTemperatures(int patch_id,
                                                std::vector<double> &foam_t) {
  if (!_solver) {
    return 0;
  }
  auto &mesh = _solver->mesh;
  auto &temp =
      mesh.boundary()[patch_id].lookupPatchField<Foam::volScalarField, double>(
          "T");
  std::copy(temp.begin(), temp.end(), std::back_inserter(foam_t));
  return temp.size();
}

std::size_t FoamSolver::patchSize(int patch_id) {
  if (!_solver) {
    return 0;
  }
  auto &mesh = _solver->mesh;
...
void FoamSolver::setPatchTemperatures(int patch_id,
                                      const std::vector<double> &moose_t) {
  if (!_solver) {
    return;
  }
  auto &mesh = _solver->mesh;
...
void FoamSolver::setPatchNegativeHeatFlux(int patch_id,
                                          std::vector<double> &negative_hf) {
  if (!_solver) {
    return;
  }
  auto &mesh = _solver->mesh;
...
std::size_t FoamSolver::wallHeatFlux(std::vector<double> &fill_vector,
                                     int patch_id) {
  if (!_solver) {
    return 0;
  }
  ...
  auto whf_dict = _solver->runTime.controlDict().lookupOrDefault(
      WALL_HEAT_FLUX, Foam::dictionary());
  auto patch = _solver->mesh.boundaryMesh()[patch_id];
  whf_dict.set("patches", Foam::wordList({patch.name()}));
  whf_dict.set("writeToFile", false);
  Foam::functionObjects::wallHeatFlux whf_func(WALL_HEAT_FLUX, _solver->runTime,
                                               whf_dict);
  whf_func.execute();

  auto wall_heat_flux =
      _solver->mesh.lookupObject<Foam::volScalarField>(WALL_HEAT_FLUX);
