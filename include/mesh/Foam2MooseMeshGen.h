...
namespace Foam {
template <typename T> class IOList;
typedef int label;
...
public:
  FoamPoint(double x0, double x1, double x2, int32_t id)
      : _pos{x0, x1, x2}, _gid(id) {};
  FoamPoint() {}

  libMesh::Point get_point() const {
    return libMesh::Point(_pos[0], _pos[1], _pos[2]);
  }
...
public:
  FoamFace(int32_t const *start, int32_t const *end, int32_t subdomain_id = 0,
           int32_t rank = 0)
...
protected:
  void calcGlobalData() {
    _globalIndex =
        _mesh->globalData().mergePoints(_pointToGlobal, _uniquePoints);
  }
...
public:
  Foam2MooseMeshAdapter(std::vector<std::string> patch_name,
                        Foam::fvMesh *fv_mesh, MPI_Comm *comm = nullptr);
...
private:
  ...
  void calc_subdom_and_rank_arrays(HippoPtr<int32_t> subdom_count,
                                   int32_t nsubdom, int32_t mpi_size);
